...
class LL_Database_TabWidget(QWidget):
    ...
    def open_cases_overview(self):
        ...
        rows = [r.row() for r in self.tabname_to_tableview[self.tabname].selectionModel().selectedRows()]
        tableview = self.tabname_to_proxy[self.tabname]
        case_paths = [tableview.index(row, 8).data() for row in rows]
        cases = [pickle.load(open(p,'rb')) for p in case_paths]
        self.parent.add_cases_overview_tab(cases, case_paths)

        
    def set_tabname(self):
        self.tabname = self.tableview_tabs.tabText(self.tableview_tabs.currentIndex())
